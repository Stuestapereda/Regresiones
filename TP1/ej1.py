"""  
1. Consideremos el siguiente vector de atributos binarios:
    (scones, cerveza, whisky, avena, fútbol)
    El vector x = (1, 0, 1, 1, 0) significa que se trata de una persona que le gustan los scones, no
    toma cerveza, le gusta el whisky y la avena pero no ve futbol. En el archivo
    PreferenciasBritanicos.xls se encuentran las preferencias de 6 personas inglesas y 7
    personas escocesas.
    
    a) Implementar el clasificador ingenuo de Bayes.
    b) Clasificar el ejemplo x1 = (1, 0, 1, 1, 0) determinando si corresponde a las preferencias
    de una persona inglesa o escocesa.
    c) Clasificar el ejemplo x2 = (0, 1, 1, 0, 1) determinando si corresponde a las preferencias
    de una persona inglesa o escocesa.
...
def Extraer_data(data):
    ...
    with open(data, 'r') as archivo:
        lector_csv = csv.reader(archivo , delimiter=';') # Abrir el archivo en modo lectura
...
Data='/Users/tomasmendietarios/Library/Mobile Documents/com~apple~CloudDocs/I.T.B.A/Cuatri/Machine Learning/Regresiones/TP1/PreferenciasBritanicos.csv'
...
def clasificar(lista, evidencia, priori, verosimilitud):
    ...
    # Normalizar las probabilidades de clase
    suma_clases = sum(clases)
    clases = [clase / suma_clases for clase in clases]
